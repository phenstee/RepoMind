...
def evaluate_rag(
    ...
    *,
    ...
    for case in suite.cases:
        ...
        case_results.append(
            RAGCaseResult(
                ...
                answer_passed=facts_passed
                and citation_recall == 1.0
                and insufficient_matches,
            )
        )
    ...
    return RAGEvaluationReport(
        ...
        case_count=count,
        mean_retrieval_recall=sum(
            result.retrieval_recall for result in case_results
        )
        / count,
        mean_context_recall=sum(result.context_recall for result in case_results)
        / count,
        mean_citation_recall=sum(result.citation_recall for result in case_results)
        / count,
